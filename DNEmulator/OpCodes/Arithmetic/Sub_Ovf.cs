...
{
    public class Sub_Ovf : OpCodeEmulator
    {
        public override Code Code => Code.Sub_Ovf;
            ...
            checked
            {
                switch (firstValue.ValueType)
                {
                    ...
                    case DNValueType.Int32 when secondValue.ValueType == DNValueType.Native:
                        ctx.Stack.Push(new NativeValue(IntPtr.Subtract(((NativeValue)secondValue).Value, ((I4Value)firstValue).Value)));
                        break;
                    ...
                    case DNValueType.Native when secondValue.ValueType == DNValueType.Int32:
                        ctx.Stack.Push(new NativeValue(IntPtr.Subtract(((NativeValue)firstValue).Value, ((I4Value)secondValue).Value)));
                        break;
                    default:
